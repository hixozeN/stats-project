...
export function calculateWN8(tankData: Partial<LestaTankStats>) {
  ...
  const rDMGc = Math.max(0, (rDMG - 0.22) / (1 - 0.22)) || 0;
  const rSPOTc = Math.max(0, Math.min(rDMGc + 0.1, (rSPOT - 0.38) / (1 - 0.38))) || 0;
  const rFRAGc = Math.max(0, Math.min(rDMGc + 0.2, (rFRAG - 0.12) / (1 - 0.12))) || 0;
  const rDEFc = Math.max(0, Math.min(rDMGc + 0.1, (rDEF - 0.10) / (1 - 0.10))) || 0;
  const rWINc = Math.max(0, (rWIN - 0.71) / (1 - 0.71)) || 0;
